...
def _build_bedrock(context, filepath, texture_override="", spawn_offset=(0.0, 0.0, 0.0), force_loop=False, existing_emitter=None):
    ...
    sim = BedrockSimulator(parser, fps=fps, duration_frames=bake_frames, force_loop=force_loop, emitter_matrices=emitter_matrices)
    particles = sim.simulate()
    ...
    if not os.path.isfile(abs_tex):
        warning = f"Texture not found: '{parser.get_texture_path()}'. Set your Resource Pack path or use manual texture."
